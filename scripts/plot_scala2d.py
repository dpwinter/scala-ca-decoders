...
def reduce_pheno(df: pd.DataFrame) -> pd.DataFrame:
    # Keep the best lifetime over reset_time at fixed (distance, p_data, p_meas, p_signal)
    group_cols = ["distance", "p_data"]
    if "p_meas" in df.columns:
        group_cols.append("p_meas")
    if "p_signal" in df.columns:
        group_cols.append("p_signal")

    if "reset_time" in df.columns:
        idx = df.groupby(group_cols)["mean_lifetime"].idxmax()
        return df.loc[idx].copy().sort_values(group_cols)

    return df.copy().sort_values(group_cols)
...
def plot_code_capacity(ax, df: pd.DataFrame) -> None:
    ...
    ax.set_ylabel(r"$p_L$")
    ax.set_title("SCALA2D code capacity")
    ax.grid(True)
    ax.legend()
...
def plot_phenomenological(ax, df: pd.DataFrame) -> None:
    ...
    ax.set_title("SCALA2D phenomenological")
    ax.grid(True)
    ax.legend()
...
def main() -> None:
    ...
    parser.add_argument("--logy-cc", action="store_true")
    parser.add_argument("--logy-pheno", action="store_true")
    ...
    cc_df = cc_df[cc_df["model"] == "scala2d"]
    ph_df = ph_df[ph_df["model"] == "scala2d"]
    ...
    if args.logy_cc:
        ax1.set_yscale("log")
    fig1.tight_layout()
    cc_path = outdir / "scala2d_code_capacity.png"
    ...
    if args.logy_pheno:
        ax2.set_yscale("log")
    fig2.tight_layout()
    ph_path = outdir / "scala2d_phenomenological.png"
